...
public:
    ...
    public:
        ...
        unsigned long long key() const { return seedOffset; }
        ...
        uint64_t getSeedOffset() const { return seedOffset; }
    ...
    int getBranchingFactor() const { return TreeWorld::branchingFactor; }
    ...
    std::vector<State> successors(const State& state) const
    {
        ...
        for (int i = 1; i <= branchingFactor; i++) {
            State succ(state.getDepth() + 1,
                       (state.getSeedOffset() * TreeWorld::branchingFactor) + i,
                       i);
    ...
    {
        string info =
          "{ \"TreeSeed\": " + to_string(seed) +
          ", \"Depth\": " + to_string(maxDepth) +
          ", \"BranchingFactor\": " + to_string(TreeWorld::branchingFactor) +
          " }";
        return info;
    }

    string getDomainName() { return "TreeWorld"; }
