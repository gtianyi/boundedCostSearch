...
using namespace std;
// TODO: This hashtable doesn't really work. Tabulation hashing results in
// non-deterministic search behavior. Very odd. Perhaps due to duplicates not
// being detected (collisions). Compare number of collisions to another, more
// basic hash function.
...
public:
    ...
    public:
        iterator(HashTable<Key, Value>* table)
            : owningTable(table)
            , e(NULL)
        {}

        iterator(HashTable<Key, Value>* table, Entry* entry)
            : owningTable(table)
            , e(entry)
        {}
    ...
    HashTable(const HashTable& rhs) { copyCode(rhs); }
    ...
    int bucketCount() { return numBuckets; }
